    ...
    <div class="layout-wrapper layout-content-navbar">
        ...
        <div class="content-wrapper">
            <div class="container-xxl flex-grow-1 container-p-y">
                <div class="card">
                    <div class="card-body">
                        <h4>Edit Social Media Website</h4><br>
                        @foreach($sosmed as $v_sosmed)
                        <form action="{{ url('/admin/proses_update_sosmed/'.$v_sosmed->id) }}" method="post" enctype="multipart/form-data">
                        @csrf
                            <div class="row">
                                <div class="col">
                                    <label><?= $v_sosmed->icon ?><?= $v_sosmed->name ?></label>
                                </div>
                                <div class="col">
                                    <input type="text" class="form-control" name="link" value="<?= $v_sosmed->link ?>">
                                </div>
                                <div class="col">
                                ...
                                <div class="col">
                                    <?php if($v_sosmed->status == 1){ ?>
                                        <form action="{{ url('/admin/status_active_sosmed/'.$v_sosmed->id) }}" method="post" enctype="multipart/form-data">
                                        @csrf
                                            <button type="submit" class="btn btn-success"><i class='bx bx-check'></i>Aktif</button>
                                        </form>
                                        <?php }else{ ?>
                                        <form action="{{ url('/admin/status_noactive_sosmed/'.$v_sosmed->id) }}" method="post" enctype="multipart/form-data">
                                        @csrf
                                            <button type="submit" class="btn btn-danger"><i class='bx bx-x' ></i>Tidak Aktif</button>
                                        </form>
                                    <?php } ?>
                                </div>
                            </div><br>
