...
<form class="d-inline" action="{{ url('/admin/delete_hotel_madinah/'.$id) }}" method="post">
    @csrf
    @method('DELETE')
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Edit Hotel Madinah</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ url('/admin/proses_update_hotel_madinah/'.$hotel->id) }}" method="post" enctype="multipart/form-data">
            @csrf
